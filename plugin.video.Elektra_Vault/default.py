...
def SKindex():
    addon_log("SKindex")
    addDir('[COLORred]###F4m Tester & Sportsdevil needed###[/COLOR]','Favorites',4,'https://seo-michael.co.uk/content/images/2015/12/sdfeat-1.png' ,  FANART,'','','','')
...
#                print channel
                ...
                try:
                    if linkedUrl=='':
                        ...
                        #print linkedUrl
                        if name == 'Celtic Tunes':
                            addDir(name.encode('utf-8'),linkedUrl.encode('utf-8'),1,thumbnail,fanArt,desc,genre,date,None,'source')
                            addDir('Sportscene','http://www.footballorgin.com/?s=sportscene',100,thumbnail,fanArt,desc,genre,date,None,'')
                        else:
                            addDir(name.encode('utf-8'),linkedUrl.encode('utf-8'),1,thumbnail,fanArt,desc,genre,date,None,'source')
                except:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('ElektraVault Select A Source', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
...
def urlsolver(url):
    ...
    if url == resolved or resolved is None:
        #import
        xbmc.executebuiltin("XBMC.Notification(Elektravault,is finding yer link.. - ,5000)")
        import urlresolver
...
def Celtic_Highlights(url):
    ...
    for img,url,name in match:
        name = (name).replace('&#8211;','-')
        Menu(name,url,101,img,FANART,'')
		
def get_link(url,img,FANART):
    List = []
    HTML = OPEN_URL(url)
    ...
    for url in match:
        ...
        Play('1st Half',play_link,102,img,FANART,'')
    for url in match2:
        Playlink = (url).replace('/v2', '').replace('zeus.json', 'video-sd.mp4?hosting_id=21772').replace('config.playwire.com', 'cdn.video.playwire.com')
        play_link = 'http:'+Playlink
        Play('2nd Half',play_link,102,img,FANART,'')
    ...
    for url in match4:
        if '2nd Half' not in List:
            if 'Full Match' not in List:
                Playlink = (url).replace('/v2', '').replace('zeus.json', 'video-sd.mp4?hosting_id=21772').replace('config.playwire.com', 'cdn.video.playwire.com')
                play_link = 'http:'+Playlink
                Play('Full Match',play_link,102,img,FANART,'')
...
if mode==None:
    ...
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
elif mode==100:Celtic_Highlights(url)
elif mode==101:get_link(url,iconimage,FANART)
elif mode==102:Resolve(url)
xbmcplugin.endOfDirectory(int(sys.argv[1]))
