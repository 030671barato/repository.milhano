...
def wizardUpdate(startup=None):
	if workingURL(WIZARDFILE):
		...
		zip = checkWizard('zip')
		if ver > VERSION:
			yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]A Milhano Build, disponibilizou uma nova versao do [COLOR %s]%s[/COLOR]!' % (COLOR2, COLOR1, ADDONTITLE), 'Gostaria de atualizar para a [COLOR %s]Versao: %s[/COLOR]?[/COLOR]' % (COLOR1, ver), nolabel='[B][COLOR red]Lembrar Depois[/COLOR][/B]', yeslabel="[B][COLOR green]Atualizar[/COLOR][/B]")
			if yes:
