...
import traceback
import cookielib
#mensagem LISTA KeletecMP
__addon__ = xbmcaddon.Addon()
__addonname__ = __addon__.getAddonInfo('name')
__icon__ = __addon__.getAddonInfo('icon')
 
msg = ("http://keltecmp-iptv.directorioforuns.com/h180-	") 
...
def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.live.KeltecMP Lists-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(KeltecMP,We failed with error code - "+str(e.code)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(KelTecMP,Novos conteúdos adicionados.,5000,"+icon+")")
        if not url is None:
...
def GetSublinks(name,url,iconimage,fanart):
    ...
    if c==1:
        try:
         ...
         rNo=dialog.select('[COLOR blue][B]KelTecMP [/B][/COLOR][COLOR red][B]IPTV[/B][/COLOR] Selecione uma opção:', List)
         if rNo>=0:
             rName=name
             rURL=str(ListU[rNo])
             try:
...
def SearchChannels():
    ...
    progress.create('KelTecMP Procurando Espere',' ')
...
def Index():
    import binascii
    import zlib
    #getData (zlib.decompress(binascii.unhexlify("")),'')
...
def urlsolver(url):
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(KelTecMP,Atualize o commonresolvers para continuar. - ,10000)")

    resolved=genesisresolvers.get(url).result
    if url == resolved or resolved is None:
        xbmc.executebuiltin("XBMC.Notification(KelTecMP,Usando Urlresolver.. - ,5000)")
        import urlresolver
...
#                    print sepate
        ...
        if addon.getSetting('ask_playlist_items') == 'true' and not queueVideo :
            ...
            for i in mu_playlist:
                ...
                iloop+=1
            dialog = xbmcgui.Dialog()
            index = dialog.select('[COLOR blue][B]KelTecMP [/B][/COLOR][COLOR red][B]IPTV[/B][/COLOR] - Selecione uma Opcao:', names)
            if index >= 0:
                playname=names[index]
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('KelTecMP','Escolha um local para salvar os arquivos.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('KelTecMP', 'Você deseja adicionar esse arquivo como fonte?')
        if ret:
...
def search(site_name,search_term=None):
    ...
    if 'youtube' in site_name:
        ...
        if res == 'True':
            ...
            xbmc.executebuiltin("XBMC.Notification(KeltecMP,No IMDB match found ,7000,"+icon+")")
...
if mode==None:
    ...
    if url:
        ...
        xbmc.executebuiltin("XBMC.Notification(KeltecMP ,Failed to extract regex. - "+"this"+",4000,"+icon+")")
elif mode==18:
    addon_log("youtubedl")
    try:
        import youtubedl
    except Exception:
        xbmc.executebuiltin("XBMC.Notification(KeltecMP,Please [COLOR yellow]install the Youtube Addon[/COLOR] module ,10000,"")")
