...
def getUrl(url, cookieJar=None,post=None, timeout=20, headers=None):
    ...
    req.add_header('User-Agent','Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, lzeus Gecko) Chrome/33.0.1750.154 Safari/537.36')
    if headers:
        for h,hv in headers:
            req.add_header(h,hv)
...
def _search(url,name):
   # print url,name
    pluginsearchurls = ['plugin.video.zeus',\
    ...
    names = ['zeus','Phoenix','SportsDevil','CanTVLive','Ccloudtv','Prosport',\
             'P2psport']
    dialog = xbmcgui.Dialog()
    index = dialog.select('Choose a video source', names)
